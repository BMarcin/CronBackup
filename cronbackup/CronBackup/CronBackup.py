...
remotes_mapping_dict = {
    's3': 'cronbackup.CronBackupRemotes.S3Remote'
}

alerts_mapping_dict = {
    'webhook': 'cronbackup.CronBackupAlerts.Webhook'
}
...
class CronBackup:
    def __init__(
        ...
        logging.debug('Initialized {} remotes and {} items'.format(len(self.config_remotes), len(self.config_items)))
    ...
    def get_target_backup_file_name(item_name: str):
        return item_name + "_" + datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '.tar.gz'
    ...
    def run(self):
        try:
            ...
            for job in jobs:
                logging.info(f'Starting a job {job["name"]}')
                # tar gz all files
